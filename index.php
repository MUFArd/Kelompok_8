<?php
session_start();
if (!isset($_SESSION['walas_id']) || $_SESSION['level'] != 'walikelas') {
...
$hari_ini = date('Y-m-d');
$id_users = $_SESSION['walas_id'];

$cek_data = mysqli_query($conn, "SELECT walas_id FROM users WHERE walas_id = '$id_users'");
$walas_id = mysqli_fetch_assoc($cek_data)['walas_id'];
$cek_kelas_jurusan = mysqli_query($conn, "SELECT * FROM walas WHERE id = '$walas_id'");
$wi = mysqli_fetch_assoc($cek_kelas_jurusan);
$kelas = $wi['kelas'];
$jurusan = $wi['jurusan'];
...
<body class="w-full h-screen bg-gradient-to-tl from-blue-300 to-white p-4 justify-center">
    ...
    <main>
        ...
        SELECT  s.id,
                ...
                a.status
        FROM siswa s
        LEFT JOIN absensi a ON s.id = a.siswa_id  AND a.tanggal = '$hari_ini'
        WHERE s.kelas = '$kelas' AND s.jurusan = '$jurusan'
        ORDER BY a.jam DESC
    ");
            ?>
                ...
                <table class="min-w-full border-collapse border border-gray-200">
                    <thead>
                        <tr class="bg-purple-500 text-white">
                            ...
                            <th class="min-w-14 text-center sm:p-1 lg:p-4 text-left">NIS</th>
                            <th class="min-w-60 sm:p-1 lg:p-4 text-left">NAMA</th>
                            <th class="min-w-14 text-center sm:p-1 lg:p-4 text-left">KELAS</th>
                            <th class="min-w-14 text-center sm:p-1 lg:p-4 text-left">JURUSAN</th>
                            <th class="min-w-40 text-center sm:p-1 lg:p-4 text-left">JAM ABSEN</th>
                            <th class="min-w-14 text-center sm:p-1 lg:p-4 text-left">STATUS</th>
                            <th class="min-w-14 text-center sm:p-1 lg:p-4 text-left">CEK</th>
                        </tr>
                    </thead>
                    <tbody>
                ...
                ");
                        ...
                        ?>
                            <tr class="even:bg-gray-100 odd:bg-white">
                                <td class="text-center sm:p-1 lg:p-8"><?= $no++ ?></td>
                                <td class="text-center sm:p-1 lg:p-8"><?= htmlspecialchars($st['nis']) ?></td>
                                <td class="sm:p-1 lg:p-8"><?= htmlspecialchars($st['nama']) ?></td>
                                <td class="text-center sm:p-1 lg:p-8"><?= htmlspecialchars($st['kelas']) ?></td>
                                <td class="text-center sm:p-1 lg:p-8"><?= htmlspecialchars($st['jurusan']) ?></td>
                                <td class="text-center sm:p-1 lg:p-8"><?= $st['jam'] ?? '-' ?></td>
                                <td class="text-center sm:p-1 lg:p-8"><?= htmlspecialchars($st['status'] ?? 'Belum Absen')  ?></td>
                                <td class="sm:p-1 lg:p-8">
                            ...
                            <tr class="total_kehadiran hidden">
                                <td colspan="8" class="p-2 bg-gray-50">
                                    <!-- Total Kehadiran (4 kolom responsif) -->
                                    ...
                                    <div class="mt-2 text-center">
                                        <?php if (empty($st['status'])): ?>
                                            <form method="POST" action="../../proses/tambah_absen_manual.php" class="flex flex-wrap justify-center gap-2">
                                                <input type="hidden" name="siswa_id" value="<?= $st['id'] ?>">
                                                ...
                                                <button name="absen" value="Alpha" class="px-3 py-1 bg-gray-500 hover:bg-gray-600 text-white rounded-md text-xs">Alpha</button>
                                            </form>
                                        <?php else: ?>
                                            <form method="POST" action="../../proses/tambah_absen_manual.php" class="flex justify-center">
                                                <input type="hidden" name="siswa_id" value="<?= $st['id'] ?>">
                                                <button name="hapus_absen" value="1" class="px-4 py-1 bg-red-600 hover:bg-red-700 text-white rounded-md text-xs">Hapus Absen</button>
                                            </form>
                                            <form method="POST" action="../../proses/simpan_to_googlesheet.php">
                                                <input type="hidden" name="id" value="<?= $st['absensi_id'] ?>">
                                                <button type="submit" class="px-3 py-1 bg-indigo-500 text-white rounded-md text-xs">Simpan</button>
                                            </form>
